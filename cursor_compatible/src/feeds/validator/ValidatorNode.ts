...
export class ValidatorNode {
  ...
  public registerSnapshot(snapshot: MarketSnapshot): void {
    try {
      ...
      logger.error(`Error validating snapshot for ${this.source}:`, error);
  ...
  private quarantine(): void {
    ...
    logger.warn(`Feed ${this.source} quarantined due to high latency or data corruption`);
  }

  private releaseFromQuarantine(): void {
    this.isQuarantined = false;
    this.quarantineStartTime = null;
    logger.info(`Feed ${this.source} released from quarantine`);
  }
} 