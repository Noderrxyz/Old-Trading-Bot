#!/usr/bin/env node
/**
 * Trust-Weighted Consensus Snapshot CLI
 * 
 * Displays current global consensus information weighted by agent trust scores.
 */

import { TrendSignalAggregator } from '../../services/global/TrendSignalAggregator.js';
import { TrustWeightedConsensusEngine } from '../../services/global/TrustWeightedConsensusEngine.js';
...
async function bootstrap() {
  console.log('Initializing trust-weighted consensus snapshot tool...');
  
  // Initialize services
  const redis = new RedisService();
  const aggregator = new TrendSignalAggregator(redis);
  const trustService = new TrustScoreService(redis);
  const consensusEngine = new TrustWeightedConsensusEngine(trustService, aggregator);
  ...
  try {
    ...
    console.log('\n🧠 TRUST-WEIGHTED GLOBAL MARKET TREND CONSENSUS 🧠\n');
    ...
    console.log('\nConsensus data reflects agent trust weights for more reliable signals.');
    console.log('Higher-trust agents have more influence on the consensus direction.');
