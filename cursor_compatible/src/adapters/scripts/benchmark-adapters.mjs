...
async function runBenchmarks() {
  ...
  const passingCount = results.filter(r => r.meetsRequirements).length;
  const successRate = (passingCount / results.length) * 100;
  
  console.log(`Chains tested: ${results.length}`);
  console.log(`Chains passing: ${passingCount}`);
  console.log(`Success rate: ${successRate.toFixed(1)}% (target: ${REQUIREMENTS.successRate}%)`);
  
  if (successRate >= REQUIREMENTS.successRate) {
    console.log('\n✅ BENCHMARK PASSED: Adapters meet performance requirements');
  } else {
    console.log('\n❌ BENCHMARK FAILED: Adapters do not meet performance requirements');
  }
  ...
  const resultsPath = path.resolve('./benchmark-results.json');
