...
exports.RiskPolicyManager = void 0;
...
class RiskPolicyManager extends events_1.EventEmitter {
    logger;
    policies;
    adjustmentHistory;
    riskEvents;
    ...
    async updateMarketCondition(condition) {
    ...
    async adjustPoliciesForMarketCondition() {
        const adjustments = [];
    ...
    async checkConstraints(metrics) {
        const events = [];
    ...
    isConstraintViolated(value, constraint) {
    ...
    mapConstraintSeverity(severity) {
        switch (severity) {
            case 'WARNING':
                return 'MEDIUM';
            case 'CRITICAL':
    ...
    async triggerEmergencyAdjustments() {
    ...
    cleanupResolvedEvents() {
        ...
        this.riskEvents = this.riskEvents.filter(event => !event.resolved || event.timestamp.getTime() > cutoffTime);
    }
    getPolicyStatus() {
        const status = [];
        for (const policy of this.policies.values()) {
            const violations = this.riskEvents.filter(e => e.policy === policy.name && !e.resolved).length;
    ...
    async overridePolicy(policyId, parameters, reason) {
    ...
    getAdjustmentHistory(limit = 100) {
        return this.adjustmentHistory.slice(-limit);
    }
    getRiskEvents(resolved = false, limit = 100) {
        return this.riskEvents
            .filter(e => resolved || !e.resolved)
            .slice(-limit);
    }
}
exports.RiskPolicyManager = RiskPolicyManager;
//# sourceMappingURL=RiskPolicyManager.js.map